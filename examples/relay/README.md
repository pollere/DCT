...
Everything on the same subnet may not be in the same trust domain (TD) and entities in the same TD may not be on the same subnet. In the former case, cState and cAdd PDUs of different TDs are differentiated by the leading trust domain id (thumbprint of the signed schema cert) component so a DeftT instance ignores PDUs with different TD ids and the TDs can share a subnet. In the latter case, we use *relays* as the *distribution* elements of a TD. Relays pass-through Publications and never decrypt them nor are they given the keys to do so but all Publication movement is subject to the schema in areceiving DeftT. The identities of relaying DeftTs must have a RLY capability in their identity chain whose argument specifies where they can "relay to" (e.g., a udp or tcp unicast link or a IPv6 link layer multicast address). A *capability* is an attribute that has a defined format and use in DeftT.

Relays are special applications that move Publications between different DeftTs (that may be attached to different network segments) whose identity bundles have the same trust anchor and compatible communications schemas. A compatible schema has the same trust root and can be identical, one a subset of the other, or each a subset of the domain schema and they overlap in a way that allows them to share some publications as well as the certificates that sign them.


Each of the relay's DeftT instances is in a different sync zone (SZ) because of being attached to a different subnet and/or using a different (sub)schema, i.e., different (sub)domain id in its PDU. cStates and cAdds are not propagated off a subnet; a *relay* moves the Publications between SZs of the same trust domain (or overlapping subdomains).  When a trust domain needs to extend beyond a single subnet, Publications are relayed between SZs using identical schemas (though different signing/encryption may be specified for the cAdds).  Signing identities can be associated with an entire TD (including subsets), but cAdd encryption is specific to a particular subnet. The same relay set up can be used to connect trust sub-domains, where some of the subnets implement only some of the domain rules in their schemas. Relays handle the three sync collections of every DeftT: **msgs** which contains the information exchanged by applications, **cert** which contains the signing identity chains of member, and **keys** which contains all the information needed for group key encryption. The **keys** collection has two sub-collections, *keys/pdus* for encryption of cAdds and *keys/msgs* for encryption of the Publications in **msgs**. Different relay DeftTs may use different PDU validators but must all use the same validators for msgs, cert, and keys Publications. Pub encryption is across a trust domain so must be passed on by relays but relays cannot join the encryption group and pass encrypted Publications without decrypting. Encryption of cAdds is local to a subnet so *keys/pdus* is never relayed and relays must be members of any local cAdd encryption group.

Current work has focused on relaying between different networks in order to extend a trust domain with the same schema everywhere and relaying between different networks where some DeftTs are using partitions of the domain schema ("subschemas").  Relays use the schema at each DeftT instance to accept or reject Publications received on other DeftTs.
...
Relays have a DeftT for each subnet, each with its own identity bundle. The shim or API for relays is a "pass-through" *ptps.hpp*. Relay DeftTs *may* have identical bundles or bundles that overlap in some way. Bundles may specify different cAdd validation methods (through pduValidators). Publications in msgs collection received on each DeftT are published to all other DeftTs if they can be validated against the schema in the destination DeftT. Certificates from each DeftT are relayed to the other DeftT's cert stores which test them against their schemas for possible storage and publication. Publications in a keys/msgs collection are checked for legitimate signing identities in the destination cert store.
...
To test relays on a single machine or a single broadcast network, subnets using different IPv6 multicast addresses can be used. If not set, a DeftT uses ff01::1234 as the default. The runAway.sh script shows setting the multicast address for the "away" entities to ff01::5678 and setting the away relay's unicast UDP port to use the local /LLM
:34567 port. For testing on a single physical subnet, ff02 can be used. To better understand relaying, run examples while monitoring communications with *dctwatch*. When (re)setting multiple default IPv6 multicast addresses, local to host or network, it can be helpful to set up a dctwatch for each by setting the DCT_MULTICAST_ADDR environment variable on each command line, e.g.,

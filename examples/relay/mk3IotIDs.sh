...
LL=default
# HALink=ff02::5612
HALinkSRVR=tcp:127.0.0.1:12345
...
# make local multicast relay capability cert (same for all)
make_cert -s $CertValidator -o locRly.cap $PubPrefix/CAP/RLY/default $RootCert
...
for nm in ${relay[@]}; do
    echo $nm$subdom
    make_cert -s $CertValidator -o $nm$subdom.cert $PubPrefix/relay/$nm$subdom locRly.cap
    make_bundle -v -o $nm$subdom.bundle $RootCert $SchemaCert locRly.cap +$nm$subdom.cert
done
...
# make link between home and apartment relay bundles - uses iote rules
# use rlyLinkS.cap for home and rlyLinkC.cap for apt if using tcp, rlyLink.cap same otherwise
# make_cert -s $CertValidator -o rlyLink.cap $PubPrefix/CAP/RLY/$HALink $KMCapCert
make_cert -s $CertValidator -o rlyLinkS.cap $PubPrefix/CAP/RLY/$HALinkSRVR $KMCapCert
make_cert -s $CertValidator -o rlyLinkC.cap $PubPrefix/CAP/RLY/$HALinkCLNT $KMCapCert
make_cert -s $CertValidator -o homeLink.cert $PubPrefix/relay/homeLink rlyLinkS.cap
make_bundle -v -o homeLink.bundle $RootCert iote.schema $KMCapCert rlyLinkS.cap +homeLink.cert
make_cert -s $CertValidator -o aptLink.cert $PubPrefix/relay/aptLink rlyLinkC.cap
make_bundle -v -o aptLink.bundle $RootCert iote.schema $KMCapCert rlyLinkC.cap +aptLink.cert

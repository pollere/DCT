#! /bin/bash
# mkIDs schema - script to create id bundles needed to run the log examples
#  'schema' is the filename of the schema's .rules file
PATH=../../../tools:$PATH

device=(frontdoor gate)
operator=(bob)
logger=(lgr)
...
# make the logger certs
for nm in ${logger[@]}; do
    make_cert -s $CertValidator -o $nm.cert $PubPrefix/logger/$nm $RootCert
done

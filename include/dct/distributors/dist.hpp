...
struct Dist {
    ...
    std::string dtype_{"dist"}; // should be set to an identifying string used in each distributor's collection name
    ...
    Dist(DirectFace& face, const Name& pPre, const Name& dPre, const certStore& cs, std::string_view d="EdDSA", std::string_view p="EdDSA") :
     ...
     {
        sync_.autoStart(false); // delay start until predecessor distributors done initializing. Set to true for cert distributor
        ...
        updateSigningKey(cs_.key(tp), cs_[tp]); // won't call the class override

       // these must be set for each distributor's needs (may be reset in specific distributor's constructors)
       // syncps's getLifetimeCb() returns this value by default.
       sync_.pubLifetime(100ms);  // needs to be reset for derived distributor if it's not this default

       // compute space for content: use worst case, the Publication with longest name
       maxContent_ -= maxName_;
       if (maxContent_ <= 0) throw ("Dist: no  space in Pub to carry content");
    }
     ...
     */
    virtual void updateSigningKey(const keyVal sk, const rData& pubCert) {
     ...
     * in use, they are chained so that setup isn't called until all the distributors
     * that need to start previously are setup (certs are always first).
     ...
     */
    virtual void setup(connectedCb&& ccb) {
    ...
    virtual void logEvent(std::string s, std::span<const uint8_t> content = {}) {
